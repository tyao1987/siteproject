...
class Util {
    ...
    public static $firstIpRegex = '/^((25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(25[0-5]|2[0-4]\d|((1\d{2})|([0-9]?\d)))$/is';
    public static $ipCountRegex = '/^[0-9]?\d$/is';
...
// 	}
	...
	static function checkUniqueColumn($column = array(),DbTable $class,$table){
	    ...
	    $result['error'] = array();
	    foreach ($column as $v){
	        $sql = "SELECT `$v`,COUNT(id) AS c FROM $table where project_id = ".$identity['project_id']." GROUP BY `$v` HAVING `$v` != '' AND c > 1";
