...
// 	}
	...
	public function paginator($conditions = array()) {
		...
		if($user['id'] != 1){
		    $this->_select->where(array('parent_id' => $user['id']));
		    $this->_select->where(array('id' => $user['id']),Predicate\PredicateSet::OP_OR);
		}
...
// 				),
		...
		if($identity['id'] == 1){
		    //ac_id
		    $wfProject = new WfProject();
		    $projectList = $wfProject->getList();
		    $selectOption = array(null=>'未选择');
		    foreach ($projectList as $project){
		        $selectOption[$project['id']] = $project['name'];
		    }
		    $form->add(array(
		        'name' => 'project_id',
		        'required'=>'required',
